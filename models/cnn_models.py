...
def discriminator(input_dim=(28,28,1), depth=64, dropout=0.4, lr=0.0004):
    model = Sequential()
    ...
    model.compile(loss='binary_crossentropy',
                  optimizer=Adam(lr=lr, beta_1=0.5, decay=calc_decay(lr)), metrics=['accuracy'])
    return model

def generator(input_dim=100, depth=64, dim=7):
    model = Sequential()
    n_nodes = depth * 7 * 7
...
def gan(discriminator, generator, gen_input_dim=100, lr=0.0002):
